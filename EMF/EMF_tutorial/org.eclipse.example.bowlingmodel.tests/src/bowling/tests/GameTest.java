...
public class GameTest extends TestCase {
	...
	public void testMatchupGameRef() {
		...
		matchup.getGame().add(game);
		assertEquals(game.getMatchup(), matchup);
	}
	/*The super class EObjects offers many methods to access an entity in 
	 * a more generic way. For example, we will test the containment between 
	 * Matchup and Game by accessing the EContainer instead of the getMatchup() method.*/
	//	public void testMatchupGameRef() {
	//		Matchup matchup = BowlingFactory.eINSTANCE.createMatchup();
	//		Game game = BowlingFactory.eINSTANCE.createGame();
	...
	public void testValidation() {
		Matchup matchup = BowlingFactory.eINSTANCE.createMatchup();
		matchup.getGame().add(BowlingFactory.eINSTANCE.createGame());
		org.eclipse.emf.common.util.Diagnostic validate = Diagnostician.INSTANCE.validate(matchup);
		assertEquals(org.eclipse.emf.common.util.Diagnostic.ERROR, validate.getSeverity());
	}
